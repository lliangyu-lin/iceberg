...
public class CreateTableAction implements Action {
  ...
  public void execute(ExecutionContext context) {
    ...
    for (int i = 0; i < schema.size(); i++) {
      SchemaField schemaField = schema.get(i);
      Type parsedType = Types.fromPrimitiveString(schemaField.type);
      Types.NestedField nf =
          schemaField.required
              ? Types.NestedField.required(i, schemaField.name, parsedType, schemaField.doc)
              : Types.NestedField.optional(i, schemaField.name, parsedType, schemaField.doc);
      fields.add(nf);
    }
